...
{
$res = sql("SELECT sign,user,aura,level,state,exp,losses,victories,money,uid,rank_i,referal_counter,referal_rcounter
 FROM `users` WHERE 
 priveleged = 0
 and
 block=''
...
$top .= ");";
$top .= "show_list ('0+');";
$f = fopen ("service/top_gamers/R".date("d-m-y").".txt","w");
fwrite($f,$top);
fclose($f);
}
?>