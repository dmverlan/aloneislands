...
$t1 = sqla("SELECT * FROM quest WHERE id = ".TOUR3."");
if(empty($_GET["show_t3"]) and !$t1["finished"]) // Турнир 1 15-99 10x10
{
	$tour3 .= "<a class=wbut href='main.php?show_t3=1'>Турнир 3</a>";
}
if(@$_GET["show_t3"] and !$t1["finished"]) // Турнир 1 15-99 10x10
{
	echo "<table border=0><tr><td><a class=but href='main.php'>Убрать</a></td><td><a class=but href='main.php?show_t3=1'>Обновить</a></td></tr></table>";
}
if(@$_GET["show_t3"] and !$t1["finished"] and $t1["type"]==0) // Турнир 1 15-99 10x10
{
	if(@$_GET["tour3"]=="join" and $pers["level"]>=15 and $pers["tour"]==0)
	{	
		set_vars("tour=3",UID);
		$pers["tour"]=3;
	}	
	...
	{
		$id = start_t1(20,1,3,TOUR3);
		sql("UPDATE quest SET finished=0,time=".tme()."+".T_START.",type=1,lParam=".$id." WHERE id = ".TOUR3);
	}
	else
	{
	...
	echo "<span style='color:#FFFFFF'><b class=lUser>Турнир 3</b> | <b>[15-50]</b> уровни | Травматичность <b>80%</b> | Таймаут <b>90 секунд</b>. | ".$start."</span>";
	...
	$users = sql("SELECT * FROM users WHERE tour=3;");
...
<br><i>Призы:</i> 10 пергаментов, 100 LN, 10000 опыта";
	...
	if($pers["level"]>=15 and $pers["tour"]==0)
		echo "<a class=gbut href='main.php?tour3=join&show_t3=1'>Вступить</a>";
	
	echo "</center>";
	echo "</div>";
	}
}
elseif(@$_GET["show_t3"] and ($t1["type"]==1 or $t1["type"]==2) and !$t1["finished"])
{
	if(!mtrunc(T_START2+$t1["time"]-tme()))
	{
		$id = start_t1(10,2,3,TOUR3);
		sql("UPDATE quest SET finished=0,time=".tme().",type=3,lParam=".$id." WHERE id = ".TOUR3);
	}
	else
	{
	$start = "";
...
elseif($t1["type"]==3 and !$t1["finished"])
	sql("UPDATE quest SET finished=1,time=".tme()." WHERE id=".TOUR3);
?>