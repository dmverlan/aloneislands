...
,1);")) echo "ПОДСКАЗКА УСПЕШНО ДОБАВЛЕНА!";
}

if (@$_GET["delete"])
{
	if (sql("DELETE FROM tips WHERE id='".$_GET["delete"]."'") and sql("UPDATE tips SET id=id-1 WHERE id>'".$_GET["delete"]."'")) 
...
if (@$_POST["edittext"])
{
	if (sql("UPDATE tips SET text='".$_POST["edittext"]."',title='".$_POST["edittitle"]."' WHERE id='".$_GET["edit"]."'")) 
	echo "<b class=brdr>Подсказка удачно изменена!</b>";
	else
	echo "<b class=brdr>Что-то не так.</b>";
}

$news = sql("SELECT * FROM tips ORDER BY `id` ASC");
...
while ($new = mysql_fetch_array($news,MYSQL_ASSOC))
{
	...
	echo "<td><a class=timef href=main.php?edit=".$new["id"].">edit</a> | <a class=timef href=main.php?delete=".$new["id"].">delete</a></td>";
	echo "</tr>";
}
echo "</table>";
...
{
	$new = sqla("SELECT * FROM tips WHERE id='".$_GET["edit"]."'");
