...
if (substr_count($pers["rank"],"<glav>"))
{
		if (empty($_POST) and empty($_GET["buy"]))
		{
			...
			$r = sql("SELECT name,id FROM wp WHERE uidp='".$pers["uid"]."' and where_buy=1 and id_in_w='' and clan_sign='".$pers["sign"]."' and weared=0");
			$i=0;
			...
			if ($i==0) echo "<div class=return_win>Клан арт должен быть снят для улучшения.</div>";
		}elseif (empty($_POST["idd"]) and empty($_GET["buy"]))
		{
			$vesh = sqla("SELECT * FROM wp WHERE id='".$_POST["id"]."' and uidp='".$pers["uid"]."' and where_buy=1 and id_in_w='' and clan_sign='".$pers["sign"]."' and weared=0");
			include("inc/inc/weapon2.php");
			$type  = $vesh["stype"];
			echo "<script>".$text."</script>";
		$s1 = '';
		for ($i=$vesh["s1"];$i<=$vesh["s1"]+3 and $i<20;$i++)
		 $s1 .= '<option value='.$i.'>+'.$i.'</option>';
		$s2 = '';
		for ($i=$vesh["s2"];$i<=$vesh["s2"]+3 and $i<20;$i++)
		 $s2 .= '<option value='.$i.'>+'.$i.'</option>';
		$s3 = '';
		for ($i=$vesh["s3"];$i<=$vesh["s3"]+3 and $i<20;$i++)
		 $s3 .= '<option value='.$i.'>+'.$i.'</option>';
		$s4 = '';
		for ($i=$vesh["s4"];$i<=$vesh["s4"]+3 and $i<20;$i++)
		 $s4 .= '<option value='.$i.'>+'.$i.'</option>';
		$s5 = '';
		for ($i=$vesh["s5"];$i<=$vesh["s5"]+3 and $i<20;$i++)
		 $s5 .= '<option value='.$i.'>+'.$i.'</option>';
		$s6 = '';
		for ($i=$vesh["s6"];$i<=$vesh["s6"]+3 and $i<20;$i++)
		...
		$udmin = '';
		if ($vesh["udmin"]<100)
		for ($i=$vesh["udmin"];$i<=$vesh["udmax"] and $i<100;$i++)
		{
		 $udmin .= '<option value='.$i.'>'.$i.'</option>';
		}
		...
		echo '<br><form method="POST" action="main.php?c=clap">
...
'.$dop.'
	...
	<tr>
		...
		}elseif (empty($_GET["buy"]))
		{
			...
			$lastid = 1+$lastid[0];
			$vesh = sqla("SELECT * FROM wp WHERE id='".$_POST["idd"]."' and uidp='".$pers["uid"]."' and where_buy=1 and id_in_w='' and clan_sign='".$pers["sign"]."'");
			$type  = $vesh["stype"];
			$oldp = $vesh["dprice"];
			$oldid = $vesh["id"];
		$dprice = 30;
...
('".$lastid."','".$vesh["name"]."','".$p["s1"]."','".$p["s2"]."','".$p["s3"]."','".$p["s5"]."','".$p["s6"]."','".$p["mf1"]."','".$p["mf2"]."','".$p["mf3"]."','".$p["mf4"]."','".$p["mf5"]."','".$p["kb"]."','".$p["hp"]."','".$p["ma"]."','".$p["udmin"]."','".$p["udmax"]."','Индивидуальный артефакт','".$dprice."','".$vesh["image"]."','".$vesh['type']."',1,'".$vesh["stype"]."','1','1','1','','".$p["slots"]."','".$p["radius"]."');
");
		...
		if ($clan["dmoney"]>=floor($dprice-$oldp))echo "<input type=button value='Усилить[".floor($dprice-$oldp)." y.e.]' class=inv_but onclick=\"location='main.php?c=clap&buy=".$lastid."&old=".$oldid."'\">";
		else echo "<input type=button value='Усилить[".abs(floor($dprice-$oldp))." y.e.]' class=inv_but DISABLED>";
		echo "<input type=button value=Отменить class=inv_but onclick=\"location='main.php?c=indap'\"><br> Усиление произойдёт за счёт клана. Усилять может лишь глава.";
		}else
		{
		$vesh = sqla("SELECT * FROM wp WHERE id='".$_GET["buy"]."'");
		$old = sqla("SELECT * FROM wp WHERE id='".$_GET["old"]."' and weared=0 and uidp=".$pers["uid"]."");
		$clan = sqla ("SELECT * FROM `clans` WHERE `sign`='".$pers['sign']."'");
		if ($old["image"]==$vesh["image"])
			{
			if ($old["dprice"] and $clan["dmoney"]>=abs($vesh["dprice"]-$old["dprice"]) and $vesh["uidp"]==0)
				{
				sql("UPDATE wp SET uidp=".$pers["uid"].",durability=1,clan_sign='".$pers["sign"]."',clan_name='".$clan["name"]."',present='".$pers["user"]."' WHERE id='".$_GET["buy"]."'");
				sql("DELETE FROM wp WHERE id='".$old["id"]."'");
				sql("UPDATE clans SET dmoney=dmoney-".abs($vesh["dprice"]-$old["dprice"])." WHERE `sign`='".$pers['sign']."'");
